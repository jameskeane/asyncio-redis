  ...
  async _run_command_queue() {
    ...
    while (this._commandQueue.length > 0) {
      ...
      if (inline) {
        ...
        this._socket.cork();
        write_to_socket(this._socket, cmd);
        this._socket.uncork();
      }
 ...
 */
function write_to_socket(socket, type) {
  if (Array.isArray(type)) {
    socket.write(`*${type.length}\r\n`);
    for (let el of type) {
      write_to_socket(socket, el);
    }
  } else if (typeof type === 'string') {
    const strBuf = Buffer.from(type);
    socket.write(`$${strBuf.length}\r\n`);
    socket.write(strBuf);
    socket.write('\r\n');
