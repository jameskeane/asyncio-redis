...
/**
 * @typedef {[string, (r: any) => void, (e: Error) => void]}
 * QueuedCommand
 */
...
class RedisClient {
  ...
  async _run_command_queue() {
    ...
    while (this._commandQueue.length > 0) {
      const [cmd, resolve, reject] = /** @type {!QueuedCommand} */ (
  ...
  info(section='default') {
    return this._serialize_command(`INFO ${section}\r\n`);
  }
   ...
   */
  set(key, val, options={}) {
    const ex_str = options.px ?
        `PX ${options.px} ` : options.ex ?
        `EX ${options.ex} ` : ``;
    const nxx_str = options.nx ? `NX` : options.xx ? `XX` : ``;

    return this._serialize_command(`SET ${key} ${val} ${ex_str}${nxx_str}\r\n`);
  }
